...
/// Screen 0001 — Onboarding paywall (RevenueCat) for **SILLY SMILES PRO**.
///
/// Geometry is taken from `source/0001.json` (375×667 pt, scale 2): a full-bleed
/// wallpaper collage hero fading into the paywall background (#0c0c0c), a
/// branded wordmark + PRO badge, three benefit rows, and two selectable plan
/// cards (335×74, 20 pt side margins) — Weekly ("Most Popular", green outline)
/// and Yearly ("Best Value") — followed by a green Continue CTA and the
/// Terms · Restore · Privacy footer links. Colors/text match `screens/0001.png`.
class OnboardingPaywallScreen extends StatefulWidget {
  const OnboardingPaywallScreen({super.key, this.onComplete});

  /// Called when the paywall is dismissed (Continue, purchase or the close X).
  /// Supplied by [ColdStartFlow] to advance the cold-start sequence to Home;
  /// null in standalone / web-preview mode, where dismissal pops or falls
...
class _OnboardingPaywallScreenState extends State<OnboardingPaywallScreen> {
  ...
  void _dismiss() {
    // Driven by the cold-start flow: advance the sequence to Home.
    ...
    if (navigator.canPop()) {
      ...
      // Standalone / web-preview entry: fall through to the app shell (Home).
      navigator.pushReplacementNamed('/');
    }
  }

  /// Continue = subscribe to the selected plan through StoreKit / RevenueCat.
  /// On a successful purchase PRO unlocks and the cold-start flow advances to
  /// Home; a failure keeps the paywall up (the close X skips it for free).
  Future<void> _subscribe() async {
    final controller = EntitlementScope.read(context);
    final unlocked = await runPurchase(
      context,
      controller,
      _selected == _Plan.weekly ? 'weekly' : 'yearly',
    );
    if (unlocked && mounted) _dismiss();
  }

  /// Restore a previous purchase; advance to Home when PRO is re-activated.
  Future<void> _restore() async {
    final unlocked = await runRestore(context, EntitlementScope.read(context));
    if (unlocked && mounted) _dismiss();
  }

  @override
  Widget build(BuildContext context) {
    // Subscribe to the entitlement so the Continue CTA reflects the StoreKit
    // round-trip spinner.
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Scrollable paywall content (hero + copy + plans + CTA + links).
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 22),
                      _buildContinueButton(entitlement.purchaseInFlight),
                      const SizedBox(height: 16),
                      _buildFooterLinks(),
          ...
          // Close (dismiss) button — 32×32 translucent circle, top-left.
          Positioned(
            top: media.padding.top + 12,
            left: 20,
            child: _CircleButton(onTap: _dismiss),
          ),
        ],
      ),
    );
  ...
  Widget _buildPlanCard({
    ...
    return GestureDetector(
      ...
      child: Padding(
        // Reserve room for the badge that straddles the top edge.
  ...
  Widget _buildContinueButton(bool busy) {
...
/// Translucent circular close button (32 pt) matching the native top-left X.
